import { GUEST_ACCESS_LIMITS } from '../../config/guestAccessLimits';
import type {
  GuestPassCode,
  GuestQuotaKind,
  PassCodeType,
...
export function validateGuestPassCode(
  ...
  if (!passCode.allowedEmails.includes(normalizeEmail(request.email) ?? '')) {
    return {
      message:
        request.email?.trim()
          ? 'This pass is not available for the email currently signed in. Please sign out and sign in with the email used when your pass was created. If you are not sure, contact the Predicta admin or pass creator.'
          : 'Please sign in with the email used when your pass was created. If you are not sure, contact the Predicta admin or pass creator.',
