...
for (const artifact of artifacts) {
  ...
  writeFileSync(artifact.text, `${extractedText}\n`);
...
function assertReportCompositionContract(result, mode) {
  ...
  for (const section of karmaSections) {
    ...
    assert.doesNotMatch(
      text,
      /implemented checks|implemented rule|implemented evidence|implemented pattern|deterministic contract|contract version|Generated by|source rules|Deduped item ids|report-only|implementation/i,
      `${mode} ${section.title} avoids internal report-engine language`,
    );
    assert.ok(
      /meaning|means|guidance|support|pressure|remedy|evidence|activation|safe|grows?|use|visible|works?|helps?/i.test(text),
...
function assertExtractedTextContract({ id, mode, pages, text }) {
  ...
  for (const forbidden of [
    ...
    /report-only/i,
    /implementation/i,
    /implemented checks/i,
    /\bimplemented\b/i,
    /implemented rule/i,
    /implemented evidence/i,
    /implemented pattern/i,
    /deterministic contract/i,
    /contract version/i,
    /Generated by/i,
    /source rules/i,
    /Deduped item ids/i,
  ...
  if (mode === 'PREMIUM') {
    assert.match(text, /Premium reading|What softens it|Remedy path|connected to \d+ chart evidence/i, `${id} premium includes deeper item/remedy detail`);
  } else {
    assert.match(text, /Free insight|Top safe remedy|safe remedy/i, `${id} free includes summary and safe remedy`);
  }
...
function renderVerification(manifest) {
  return [
    ...
    '- Report uses composeKundliKarmaSnapshot from the Kundli Karma intelligence packet.',
    ...
    '- Premium artifact includes deeper item detail, activation, reductions, remedy path, chart-evidence linkage, and consolidated remedy plan.',
