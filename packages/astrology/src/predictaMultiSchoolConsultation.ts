...
function nativeConfidence(language: SupportedLanguage, label: string): string {
  if (/high/i.test(label)) {
    return nativeConsultationCopy(`astrology.common.confidence.high.${language}`, label);
  }
  if (/low|not enough/i.test(label)) {
    return nativeConsultationCopy(`astrology.common.confidence.low.${language}`, label);
  }
  return nativeConsultationCopy(`astrology.common.confidence.medium.${language}`, label);
}

function nativeEventTimingHonesty(language: SupportedLanguage): string {
  return nativeConsultationCopy(
    `astrology.predictaMultiSchool.eventTimingHonesty.${language}`,
    'The timing signal is read with dasha, transit, and event evidence; do not treat it as a guaranteed date.',
  );
}

function nativeConflictRespect(language: SupportedLanguage): string {
  return nativeConsultationCopy(
    `astrology.predictaMultiSchool.conflictRespect.${language}`,
    'Some signals ask for caution, so Predicta is not forcing the answer into false certainty.',
  );
}
...
function nativeEventDirectAnswer(
  ...
  return nativeConsultationCopy(
    `astrology.predictaMultiSchool.eventDirectAnswer.${categoryId}.${language}`,
    'A meaningful event signal is possible, but it should be handled with preparation and timing awareness.',
  );
}
...
function nativeEventTrigger(
  ...
  if (categoryId === 'foreign_travel' || categoryId === 'visa_pr') {
    return nativeConsultationCopy(
      `astrology.predictaMultiSchool.eventTrigger.foreign_travel.${language}`,
      'Work need, documents, a senior person, team change, or travel approval.',
    );
  }
  if (categoryId === 'career_move' || categoryId === 'job_change' || categoryId === 'promotion') {
    return nativeConsultationCopy(
      `astrology.predictaMultiSchool.eventTrigger.career_move.${language}`,
      'Manager change, role opening, review cycle, recruiter contact, or visible work.',
    );
  }
  if (categoryId === 'marriage_timing' || categoryId === 'relationship_outcome') {
    return nativeConsultationCopy(
      `astrology.predictaMultiSchool.eventTrigger.marriage_timing.${language}`,
      'Serious conversation, family participation, proposal, reconciliation, or commitment decision.',
    );
  }
  if (categoryId === 'money_property') {
    return nativeConsultationCopy(
      `astrology.predictaMultiSchool.eventTrigger.money_property.${language}`,
      'Paperwork, loan update, buyer/seller response, cash clarity, or a deadline.',
    );
  }
  return nativeConsultationCopy(
    `astrology.predictaMultiSchool.eventTrigger.default.${language}`,
    'Practical opportunity, conversation, approval, responsibility, or a deadline.',
  );
}
...
function nativeEventAction(
  ...
  if (categoryId === 'money_property') {
    return nativeConsultationCopy(
      `astrology.predictaMultiSchool.eventAction.money_property.${language}`,
      'Check documents, budget, and cash safety before taking the next step.',
    );
  }
  if (categoryId === 'foreign_travel' || categoryId === 'visa_pr') {
    return nativeConsultationCopy(
      `astrology.predictaMultiSchool.eventAction.foreign_travel.${language}`,
      'Keep documents ready and watch for signals from work or institution.',
    );
  }
  if (categoryId === 'marriage_timing' || categoryId === 'relationship_outcome') {
    return nativeConsultationCopy(
      `astrology.predictaMultiSchool.eventAction.marriage_timing.${language}`,
      'Look for clear conversation and consistent behavior before emotion takes over.',
    );
  }
  return nativeConsultationCopy(
    `astrology.predictaMultiSchool.eventAction.default.${language}`,
    'Stay prepared, avoid rushing, and take practical steps when the signal appears.',
  );
}

function nativeConsultationCopy(key: string, fallback: string): string {
