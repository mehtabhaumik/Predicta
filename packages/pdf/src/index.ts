...
function buildKundliKarmaReportSections(
  ...
  const summaryBody = topCondition
    ? `${topCondition.item.meaningForUser} Predicta reads this first because ${topCondition.whyThisRankedFirst}`
    : 'Predicta did not find a major active Kundli Karma condition in the implemented checks. That means the report should focus on steady habits, clear timing, and practical support instead of raising fear.';
  ...
  return [
    {
      ...
      bullets: [
        activeNames.length
          ...
          : 'No major active Kundli Karma alerts were ranked from the implemented deterministic checks.',
        snapshot.strongestDosh
          ? `Strongest Dosh: ${snapshot.strongestDosh.item.displayName} - ${snapshot.strongestDosh.item.meaningForUser}`
          : 'Strongest Dosh: no major active Dosh ranked from the implemented checks.',
        snapshot.strongestShrapOrRin
          ...
          : 'Strongest Shrap/Rin indicator: no major active indicator ranked from the implemented checks.',
        snapshot.strongestYog
          ? `Strongest Yog: ${snapshot.strongestYog.item.displayName} - ${snapshot.strongestYog.item.meaningForUser}`
          : 'Strongest Yog: no major active Yog ranked from the implemented checks.',
        topRemedy
          ? `Top safe remedy: ${topRemedy.description}`
          : 'Top safe remedy: keep one simple karma/dharma action before adding rituals.',
      ],
      evidence: [
        `Shared deterministic contract version: ${snapshot.version}.`,
        `Generated by: ${snapshot.generatedBy}.`,
        `Calculation status: ${snapshot.calculationStatus}.`,
        `Ranked conditions in shared snapshot: ${snapshot.rankedConditions.map(condition => condition.item.displayName).join(', ') || 'none'}.`,
        `Deduped item ids: ${snapshot.dedupedItemIds.join(', ') || 'none'}.`,
    ...
    buildKundliKarmaModuleSection({
      ...
      emptyMessage: 'Predicta does not raise a major Dosh from the implemented deterministic checks. Use this as a calm sign to focus on ordinary discipline rather than fear.',
      eyebrow: 'DOSH',
    }),
    buildKundliKarmaModuleSection({
      mode,
      ...
      emptyMessage: 'Predicta does not raise a major Shrap indicator from the implemented checks. This layer stays calm and only speaks when evidence is visible.',
      eyebrow: 'SHRAP',
    }),
    buildKundliKarmaModuleSection({
      mode,
      ...
      rankedConditions: snapshot.rankedConditions,
      title: 'Positive Yog',
      emptyMessage: 'Predicta did not rank a major supportive Yog from the implemented checks. Guidance: build strength through house support, dasha timing, steady habits, and daily choices instead of waiting for one label to carry the whole life.',
    ...
    buildKundliKarmaModuleSection({
      ...
      emptyMessage: 'Predicta did not rank a major challenging Yog from the implemented checks. Guidance: do not invent fear; use steady routine, clean decisions, and the active Mahadasha section for timing pressure instead.',
    ...
    buildKundliKarmaModuleSection({
      ...
      emptyMessage: 'Predicta did not rank a major Lal Kitab observation from the implemented checks. Lal Kitab remedies remain one-at-a-time and safe.',
      eyebrow: 'LAL KITAB',
    }),
  ...
  return {
    body: first
      ? `${first.meaningForUser} ${
          isPremium
            ? `Premium detail: ${kundliKarmaPremiumDetail(first)}`
            : `Free summary: ${first.summary}`
        }`
      : emptyMessage,
    ...
    evidence: visible.flatMap(condition => [
      `${condition.item.displayName}: status ${formatKundliKarmaStatus(condition.item.status)}, strength ${condition.item.strength}, confidence ${condition.item.confidence}.`,
      `Why present: ${condition.item.whyPresent}`,
      `Activation: ${condition.item.activation.summary}`,
      ...condition.item.reductions.slice(0, isPremium ? 3 : 1).map(reduction => `Reduction: ${reduction.description}`),
      ...condition.item.crossReferences
        .filter(reference => reference.relationship === 'do_not_duplicate')
        .slice(0, 2)
        .map(reference => `Cross-reference: ${reference.note}`),
    ]),
...
function buildKundliKarmaRemedyPlanSection(
  ...
  return {
    body: first
      ? `Guidance: ${first.description} This is the consolidated plan, so repeated Dosh, Shrap, Yog, and Lal Kitab remedies are merged instead of scattered across the report.`
      : 'Guidance: no separate remedy list is inflated here. Predicta keeps remedies simple, safe, low-cost, and based on visible chart evidence.',
    bullets: dedupedVisible.length
      ? dedupedVisible.map(item => `${item.title}: ${item.description} Timing: ${item.timingGuidance}`)
      : ['No active remedy plan item was produced by the deterministic contract.'],
    evidence: dedupedVisible.flatMap(item => [
      `${item.title}: source rules ${item.sourceRuleIds.join(', ') || 'none'}.`,
...
function formatKundliKarmaConditionBullet(
  ...
  return [
    ...
    reduction ? `What reduces it: ${reduction}` : 'What reduces it: no major cancellation evidence is active in this contract.',
...
function kundliKarmaPremiumDetail(item: KundliKarmaItem): string {
  ...
  const reductions = item.reductions.length
    ...
    : 'No strong cancellation evidence is active in this contract.';
  const premiumRemedy =
