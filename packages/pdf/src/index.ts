  ...
  return {
    ...
    evidenceTable: [
      ...
      },
      {
        confidence: 'high',
        factor: 'Timing',
        implication: 'The current dasha frames the active timing and what needs attention now.',
    ...
    evidenceTable: [
      {
        ...
        implication: 'The reading begins with what the Kundli means, then keeps the proof visible for trust.',
      },
      {
        confidence: 'high',
...
function buildVedicSnapshotReportSection(
  ...
  return {
    body:
      `${vedicSectionMeaning(snapshot, mode)} The birth snapshot behind this reading is ${snapshot.lagna} Lagna, ${snapshot.moonSign} Moon, ${snapshot.nakshatra} nakshatra, and ${snapshot.currentDasha}.`,
    bullets: [
...
function buildKundliKarmaReportSections(
  ...
  const ranked = isPremium ? snapshot.rankedConditions : snapshot.rankedConditions.slice(0, 3);
  const activeNames = ranked.map(condition => condition.item.displayName);
  const topCondition = ranked[0];
  const topRemedy = snapshot.topRemedy;
  const summaryBody = topCondition
    ? `${topCondition.item.meaningForUser} Predicta brings this forward first because it is the strongest visible karma signal in this chart right now: ${describeKundliKarmaRankReason(topCondition)}`
    : 'Predicta did not find a major active Kundli Karma condition in the current chart evidence. That is a calm sign to focus on steady habits, clear timing, and practical support instead of raising fear.';
  ...
  return [
    {
      ...
      bullets: [
        activeNames.length
          ? `Top conditions to review first: ${activeNames.join(', ')}.`
          : 'No major Kundli Karma alerts are active from the visible chart evidence.',
        snapshot.strongestDosh
          ? `Strongest Dosh: ${snapshot.strongestDosh.item.displayName} - ${snapshot.strongestDosh.item.meaningForUser}`
          : 'Strongest Dosh: no major active Dosh is visible in the current chart evidence.',
        snapshot.strongestShrapOrRin
          ? `Strongest Shrap/Rin indicator: ${snapshot.strongestShrapOrRin.item.displayName} - ${snapshot.strongestShrapOrRin.item.meaningForUser}`
          : 'Strongest Shrap/Rin indicator: no major active indicator is visible in the current chart evidence.',
        snapshot.strongestYog
          ? `Strongest Yog: ${snapshot.strongestYog.item.displayName} - ${snapshot.strongestYog.item.meaningForUser}`
          : 'Strongest Yog: no major active Yog is visible in the current chart evidence.',
        topRemedy
          ? `Top safe remedy: ${topRemedy.description}`
          : 'Top safe remedy: keep one simple karma/dharma action before adding rituals.',
      ],
      evidence: [
        `Evidence status: ${snapshot.calculationStatus}.`,
        `Conditions reviewed in this reading: ${snapshot.rankedConditions.map(condition => condition.item.displayName).join(', ') || 'none'}.`,
        'Repeated remedies and overlapping conditions are merged so the report gives one clean action plan.',
        ...snapshot.safetyNotes,
      ],
      evidenceTable: ranked.slice(0, isPremium ? 6 : 3).map(condition => kundliKarmaConditionToEvidenceRow(condition, mode)),
    ...
    buildKundliKarmaModuleSection({
      ...
      title: 'Dosh In Your Kundli',
      emptyMessage: 'Predicta does not raise a major Dosh from the current chart evidence. Use this as a calm sign to focus on ordinary discipline rather than fear.',
      eyebrow: 'DOSH',
    }),
    buildKundliKarmaModuleSection({
      mode,
      module: 'SHRAP',
      rankedConditions: snapshot.rankedConditions,
      title: 'Karmic Debt & Shrap Indicators',
      emptyMessage: 'Predicta does not raise a major Shrap indicator from the current chart evidence. This layer stays calm and only speaks when evidence is visible.',
      eyebrow: 'SHRAP',
    }),
    buildKundliKarmaModuleSection({
      mode,
      ...
      rankedConditions: snapshot.rankedConditions,
      title: 'Positive Yog',
      emptyMessage: 'Predicta does not raise a major supportive Yog from the current chart evidence. Build strength through house support, dasha timing, steady habits, and daily choices instead of waiting for one label to carry the whole life.',
      eyebrow: 'POSITIVE YOG',
    }),
    buildKundliKarmaModuleSection({
      mode,
      module: 'CHALLENGING_YOG',
      rankedConditions: snapshot.rankedConditions,
      title: 'Challenging Yog',
      emptyMessage: 'Predicta does not raise a major challenging Yog from the current chart evidence. Use steady routine, clean decisions, and the active Mahadasha section for timing pressure instead.',
      eyebrow: 'CHALLENGING YOG',
    }),
    buildKundliKarmaModuleSection({
      mode,
      module: 'LAL_KITAB',
      ...
      emptyMessage: 'Predicta does not raise a major Lal Kitab observation from the current chart evidence. Lal Kitab remedies remain one-at-a-time and safe.',
      eyebrow: 'LAL KITAB',
    }),
    buildKundliKarmaRemedyPlanSection(snapshot.remedyPlan, mode),
  ];
}

function buildKundliKarmaModuleSection({
  emptyMessage,
  eyebrow,
  mode,
  module,
  ...
  eyebrow: string;
  mode: PDFMode;
  module: KundliKarmaItem['module'];
  rankedConditions: KundliKarmaRankedCondition[];
  title: string;
}): PdfSection {
  const isPremium = mode === 'PREMIUM';
  const moduleConditions = rankedConditions.filter(condition => condition.item.module === module);
  const visible = isPremium ? moduleConditions : moduleConditions.slice(0, 3);
  const first = visible[0]?.item;

  return {
    body: first
      ? `${first.meaningForUser} ${
          isPremium
            ? `Premium reading: ${kundliKarmaPremiumDetail(first)}`
            : `Free insight: ${first.summary}`
        }`
      : emptyMessage,
    bullets: visible.length
      ? visible.map(condition => formatKundliKarmaConditionBullet(condition, mode))
      : [emptyMessage],
    evidence: visible.flatMap(condition => [
      `${condition.item.displayName}: ${formatKundliKarmaStatus(condition.item.status)}, ${condition.item.strength} strength, ${condition.item.confidence} confidence.`,
      `Why this appears: ${condition.item.whyPresent}`,
      `Activation: ${condition.item.activation.summary}`,
      ...condition.item.reductions.slice(0, isPremium ? 3 : 1).map(reduction => `What softens it: ${reduction.description}`),
      ...condition.item.crossReferences
        .filter(reference => reference.relationship === 'do_not_duplicate')
        .slice(0, 2)
        .map(reference => `Overlap note: ${reference.note}`),
    ]),
    evidenceTable: visible.map(condition => kundliKarmaConditionToEvidenceRow(condition, mode)),
    eyebrow,
    tier: isPremium ? 'premium' : 'free',
    title,
  };
...
function buildKundliKarmaRemedyPlanSection(
  remedyPlan: KundliKarmaRemedyPlanItem[],
  mode: PDFMode,
): PdfSection {
  const isPremium = mode === 'PREMIUM';
  const visible = isPremium ? remedyPlan : remedyPlan.filter(item => item.depth === 'free').slice(0, 3);
  const dedupedVisible = dedupeKundliKarmaRemedyPlan(visible);
  const first = dedupedVisible[0];

  return {
    body: first
      ? `${first.description} This is the consolidated plan, so repeated Dosh, Shrap, Yog, and Lal Kitab remedies are merged instead of scattered across the report.`
      : 'Predicta keeps remedies simple, safe, low-cost, and based on visible chart evidence instead of inflating a separate remedy list.',
    bullets: dedupedVisible.length
      ? dedupedVisible.map(item => `${item.title}: ${item.description} Timing: ${item.timingGuidance}`)
      : ['No active remedy plan item is needed from the visible Kundli Karma evidence.'],
    evidence: dedupedVisible.flatMap(item => [
      `${item.title}: connected to ${item.sourceRuleIds.length} chart evidence line${item.sourceRuleIds.length === 1 ? '' : 's'}.`,
      `Safety: ${item.safetyNote}`,
      ...(item.avoidList.length ? [`Avoid: ${item.avoidList.join('; ')}`] : []),
    ]),
    evidenceTable: dedupedVisible.map(item => ({
      confidence: item.sourceRuleIds.length ? 'high' : 'medium',
      factor: item.title,
      observation: item.description,
      implication: `${item.timingGuidance} ${item.safetyNote}`,
    })),
    eyebrow: 'KUNDLI KARMA REMEDY',
    tier: isPremium ? 'premium' : 'free',
    title: 'One Consolidated Remedy Plan',
  };
}

function describeKundliKarmaRankReason(
  condition: KundliKarmaRankedCondition,
): string {
  const item = condition.item;
  const status = formatKundliKarmaStatus(item.status);
  const activation = item.activation.summary;

  return `${item.displayName} is ${status}, carries ${item.strength} strength, has ${item.confidence} confidence, and becomes most useful to handle through this timing: ${activation}.`;
}

function kundliKarmaConditionToEvidenceRow(
  condition: KundliKarmaRankedCondition,
  mode: PDFMode,
): PdfEvidenceRow {
  const item = condition.item;
  const evidence = item.evidence.slice(0, mode === 'PREMIUM' ? 3 : 1);

  return {
    confidence: item.confidence === 'clear' ? 'high' : item.confidence === 'partial' ? 'medium' : 'low',
    factor: `${condition.rank}. ${item.displayName}`,
    observation: evidence.length
      ? evidence.map(formatKundliKarmaEvidence).join(' ')
      : item.whyPresent,
    implication: item.meaningForUser,
  };
}

function formatKundliKarmaConditionBullet(
  condition: KundliKarmaRankedCondition,
  mode: PDFMode,
): string {
  const item = condition.item;
  const remedy = pickKundliKarmaRemedy(item, mode);
  const reduction = item.reductions[0]?.description;
  const activation = item.activation.summary;

  return [
    `${condition.rank}. ${item.displayName}: ${item.meaningForUser}`,
    `Evidence: ${item.whyPresent}`,
    `Activation: ${activation}`,
    reduction ? `What reduces it: ${reduction}` : 'What reduces it: no major cancellation evidence is active in this chart.',
    remedy ? `Remedy: ${remedy.description}` : 'Remedy: keep the correction practical and non-fearful.',
  ].join(' ');
}

function kundliKarmaPremiumDetail(item: KundliKarmaItem): string {
  const evidence = item.evidence.slice(0, 3).map(formatKundliKarmaEvidence).join(' ');
  const reductions = item.reductions.length
    ? item.reductions.slice(0, 2).map(reduction => reduction.description).join(' ')
    : 'No strong cancellation evidence is active in this chart.';
  const premiumRemedy =
    item.remedies.find(remedy => remedy.depth === 'premium') ??
    item.remedies.find(remedy => remedy.depth === 'free');

  return [
    `Why present: ${item.whyPresent}`,
    evidence ? `Evidence: ${evidence}` : '',
    `Activation: ${item.activation.summary}`,
    `What softens it: ${reductions}`,
    premiumRemedy ? `Remedy path: ${premiumRemedy.description}` : '',
  ].filter(Boolean).join(' ');
}

function formatKundliKarmaEvidence(evidence: KundliKarmaItem['evidence'][number]): string {
  return [
    evidence.description,
    evidence.planet ? `Planet: ${evidence.planet}.` : '',
    evidence.relatedPlanet ? `Related planet: ${evidence.relatedPlanet}.` : '',
    evidence.house ? `House ${evidence.house}.` : '',
    evidence.sign ? `${evidence.sign}.` : '',
    typeof evidence.degree === 'number' ? `${formatDegree(evidence.degree)}.` : '',
    evidence.nakshatra ? `${evidence.nakshatra}${evidence.pada ? ` pada ${evidence.pada}` : ''}.` : '',
  ].filter(Boolean).join(' ');
}

function pickKundliKarmaRemedy(
  item: KundliKarmaItem,
  mode: PDFMode,
): KundliKarmaItem['remedies'][number] | undefined {
  if (mode === 'PREMIUM') {
    return item.remedies.find(remedy => remedy.depth === 'premium') ?? item.remedies[0];
  }

  return item.remedies.find(remedy => remedy.depth === 'free') ?? item.remedies[0];
}
...
function dedupeKundliKarmaRemedyPlan(
  remedyPlan: KundliKarmaRemedyPlanItem[],
): KundliKarmaRemedyPlanItem[] {
  const seen = new Set<string>();
  return remedyPlan.filter(item => {
...
function buildClassicalVedicReportSection(
  ...
  return {
    body: section.status === 'ready'
      ? `${vedicSectionMeaning(section, mode)} Supporting note: ${section.explanation}`
    ...
    bullets: uniqueValues([
      ...
      section.limitations[0] ? `Limit: ${section.limitations[0]}` : 'Supporting evidence is available for this layer.',
    ]),
    evidence: sectionEvidence(section),
...
function buildFullJyotishCoverageSection(kundli: KundliData, mode: PDFMode): PdfSection {
  ...
  return {
    ...
    bullets: [
      ...
      `Yog patterns and care factors included: ${kundli.yogas.length ? `${kundli.yogas.length} recognized patterns.` : 'no recognized patterns in this Kundli.'}`,
...
function buildLimitationsSection(kundli: KundliData, mode: PDFMode): PdfSection {
  ...
  return {
    body: 'A trustworthy report names its limits. This protects the user from false certainty and keeps the reading grounded.',
    bullets: limitations,
    evidence: [
